...
\begin{enumerate}
  ...
  Ответ: Нет.\\
  \begin{enumerate} 
     \item Для объединения.\\
     Возьмем в качестве первого языка $L_1 = 1^{2n}$ и в качестве второго $L_2 = 1^{2n + 1}$\\
     Min ДКА для $L_1$:
     \begin{myauto}
     	...
     	\path[->]
     	(S) edge [bend left=15] node [above] {$1$} (T)
     	(T) edge [bend left=15] node [below] {$1$} (S)
     	;
     \end{myauto}
     Min ДКА для $L_2$:
     \begin{myauto}
     ...
     Их произведение для объединения:
	 \begin{myauto}
	 	\node [state, initial, accepting] (Q11)                {$S_1T_1$};
	 	\node [state, accepting]          (Q21) [below=of Q11] {$S_2T_1$};
	 	\node [state, accepting]          (Q12) [right=of Q11] {$S_1T_2$};
	 	\node [state, accepting]          (Q22) [right=of Q21] {$S_2T_2$};
	 	
	 	\path[->]
	 	(Q11) edge [bend left=15] node [above] {$1$} (Q22)
	 	(Q22) edge [bend left=15] node [below] {$1$} (Q11)
	 	(Q12) edge [bend left=15] node [above] {$1$} (Q21)
	 	(Q21) edge [bend left=15] node [below] {$1$} (Q12)
	 	;
	 \end{myauto}
     Min ДКА для объединения:
     \begin{myauto}
     	\node [state, initial, accepting] (S) {$S$};
     	
     	\path[->]
     	(S) edge [loop above] node [above] {$1$} ()
     	;
     \end{myauto}
 	 \item Для пересечения:\\
 	 Возьмем такие же $L_1$ и $L_2$.\\
 	 Их произведение для пересечения:
	 \begin{myauto}
	 	\node [state, initial]   (Q11)                {$S_1T_1$};
	 	\node [state] (Q21) [below=of Q11] {$S_2T_1$};
	 	\node [state, accepting] (Q12) [right=of Q11] {$S_1T_2$};
	 	\node [state]            (Q22) [right=of Q21] {$S_2T_2$};
	 ...
	 \end{myauto}
 	 ...
 	 Min ДКА для пересечения:
  	 ...
  	 \end{myauto}
     \item Для разности:\\
     Возьмем произведение двух автоматов для языка $L = 1^{2n + 1}$.\\
     Их произведение:
      \begin{myauto}
      	\node [state, initial]   (Q11)                {$S_1T_1$};
      	\node [state, accepting] (Q21) [below=of Q11] {$S_2T_1$};
      	\node [state]            (Q12) [right=of Q11] {$S_1T_2$};
      	\node [state]            (Q22) [right=of Q21] {$S_2T_2$};
  ...
  {\bf Решение}\\
  Эта регулярка эквивалетна $(a \mid b)^+ (a \mid b)^+$.
  \begin{enumerate}
    ...
    {\bf Решение}\\
    	\begin{myauto}
    		...
    		\node [state, accepting] (q_{2}) [right=of q_{1}] {$q_{t}$};
    		
    		\path[->]
    		(q_{0}) edge              node [above] {$a, b$} (q_{1})
    		(q_{1}) edge [loop above] node [above] {$a, b$} ()
    		(q_{1}) edge              node [above] {$a, b$} (q_{2})
    		(q_{2}) edge [loop above] node [above] {$a, b$} ()
    		;
    	\end{myauto}
  ...
  {\bf Решение}\\
  Пусть L - автоматный. Тогда для него верна лемма о накачке.\\
  Возьмем n из леммы и возьмем слово $\alpha = (1)^n00(1)^n$.\\
  Мы знаем что есть слова $u, v, w \in L$, такие что $|uv| \le n, |v| \ge 1$\\
  Тогда $|v| = \{1\}^+$. Рассмотрим слово $uvvw = (1)^{n + |v|}00(1)^n$, оно должно лежать в языке по лемме.\\
  Но, с другой стороны, это слово не является палиндромом.\\
  Противоречие.
  ...
  {\bf Решение}\\
  Пусть L - автоматный. Тогда для него верна лемма о накачке.\\
  Возьмем n из леммы и возьмем слово $\alpha = (b)^naa(ba)^n$.\\
  Мы знаем что есть слова $u, v, w \in L$, такие что $|uv| \le n, |v| \ge 1$\\
  Тогда $v \in \{b\}^+$. Рассмотрим $uw = (b)^{n - |v|}aa(ba)^n$, оно должно лежать в языке по лемме.\\
  Но, с другой стороны, его нельзя представить в виде двух слов U и V, что $uw = UaaV,\ |U|_b \ge |U|_a$, так как  $(n - |v| = |U|_b < |U|_a = n)$.\\
  Противоречие.
\end{enumerate}
